...
#' @description Mainly used to produce glucometrics table
...
ProGluTable <- function(metricList, unitVal){
  ...
  #            )
HyperFreqNR <- rbind(sapply(1:3,function(x) {return(metricList[[x]][grepl("Hyper1st",names(metricList[[x]]))])}),
...
TotFreq <- rbind(sapply(1:3,
                        function(x){
                          return(metricList[[x]][grepl("Reading",names(metricList[[x]]))])
                          }))
HypoFreq <- rbind(sapply(1:3,function(x) {return(metricList[[x]][grepl("Hypo1st",names(metricList[[x]]))])}),
...
c1_upd <- c1
## Modification done on 23 June 2018 by Chen Ying
## median: report IQR in the bracket
## mean: report SD in the bracket
## rounding to integer
eTab <- round(rbind(sapply(2:3,function(x) {return(metricList[[x]][grepl("Mmedian",names(metricList[[x]]))])}),
...
cTab1 <- round(rbind(sapply(2:3,function(x) {return(metricList[[x]][grepl("M1Q",names(metricList[[x]]))])}),
...
cTab2 <- round(rbind(sapply(2:3,function(x) {return(metricList[[x]][grepl("M3Q",names(metricList[[x]]))])}),
...
recur <- paste0(metricList[[3]][grepl("RecurHypo",names(metricList[[3]]))],
               " (",round(
                 metricList[[3]][grepl("RecurHypo",names(metricList[[3]]))]/metricList[[3]][grepl("FreqReading",names(metricList[[3]]))]*100
               , round_digits_p),"%)")
table1 <- rbind(TotFreq,
                matrix(paste0(HyperFreqNR, " (",round(HyperPropNR*100, round_digits_p),"%)"),nrow(HyperFreqNR),ncol(HyperFreqNR),byrow = FALSE)[1:3,],
                c(rep("",2),finc2[3]),
                c(rep("",2),finc2[4]),# HGI median and mean
                matrix(paste0(HyperFreqNR, " (",round(HyperPropNR*100, round_digits_p),"%)"),nrow(HyperFreqNR),ncol(HyperFreqNR),byrow = FALSE)[4,], # normal range
                ...
                matrix(paste0(HypoFreq, " (",round(HypoProp*100, round_digits_p),"%)"),nrow(HypoFreq),ncol(HypoFreq),byrow = FALSE),
                c(rep("",2),recur),
                cbind(rep("",4),finTab[3:6,])[1:2,],
                cbind(rep("",4),finTab[3:6,])[3:4,])

message <-c("*:The summary statistics of the glucometrics for patient-day means and patient-stay means.",
            paste0("#1: ",metricList[[2]]["NAinSD"]," (",round(metricList[[2]]["NAinSD"]/metricList[[2]]["FreqReading"]*100,2),
                   "%) patient-days were removed for calculation of patient-day SD and J-index."),
            paste0("#2: ",metricList[[3]]["NAinSD"]," (",round(metricList[[3]]["NAinSD"]/metricList[[3]]["FreqReading"]*100,2),
